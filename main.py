...
def main(GCP, job_dir):
    ...
    training_generator = DataGenerator(
        datafile='{}train.json'.format(data_folder), data_path='{}img_big/train/'.format(data_folder),
        batch_size=64, shuffle=True)

    validation_generator = DataGenerator(
        datafile='{}validation.json'.format(data_folder), data_path='{}img_big/validation/'.format(data_folder),
        batch_size=64, shuffle=True)
...
def make_local_preds():
    global model_name
    global model_class
    global save_images

    model_name = "../results/models/Xception_occ100000.h5"

    model = load_model(model_name)

    val_generator = DataGenerator(datafile="../data/validation.json", data_path="../data/img/validation", batch_size=64)

    y_pred = model.predict_generator(val_generator,
                                     steps=None,
                                     verbose=1)
    np.save("y_val", y_pred)
